...
export async function extractTattooLead({ phone, history, latestMessage, mediaItems = [] }) {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `Extract a structured tattoo lead from the conversation. Return only valid JSON matching this schema: ${JSON.stringify(LEAD_SCHEMA)}. Do not invent missing information. Use null for unknown fields. hasReferenceImage must be true if mediaItems contains image media or the conversation says a reference was sent. missingFields must check: name, tattooIdea, bodyPlacement, approximateSize, desiredStyle, availability, isFirstTattoo. leadStatus rules: new = very little info; incomplete = useful info exists but key fields missing; qualified = enough info for studio review; needs_human = medical advice, complex pricing, complaints, urgent or sensitive topics.`
      },
      {
        role: "user",
